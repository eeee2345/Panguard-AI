...
The trust-page promise becomes runnable, and one Threat Cloud answer means one
answer.

- **`pga report sign verify` ships in the free CLI.** The /trust/signing-key
  page has instructed auditors to verify issued reports offline since July; the
  command now exists and speaks the issuer's actual wire format: PGA-SIG-V1
  context-bound Ed25519 signature over the hashSchema-v2 full-body canonical
  hash. Trust policy follows the published statement — the signing key_id must
  be on the published list, or explicitly pinned with `--expect-key`. Cross-repo
  parity with the issuer tooling is pinned by a fixture generated by the issuer
  code itself. Verification is free and offline; issuing stays with the issuer.
- **Consenting to Threat Cloud now wires all three flags.** One "yes" enables
  telemetry, upload, and rule sync together (previously only part of the intent
  persisted, so agreeing endpoints still uploaded nothing), and the prompt no
  longer argues against itself. Existing installs are untouched — saved configs
  are never flipped retroactively.
- **Evidence pack self-hash covers the full document (v1.2).** The exported
  `attestation.sha256` previously hashed only the top-level skeleton (a
  `JSON.stringify` replacer-array bug), so nested verdict edits survived a
  recompute. It is now a deep canonical hash, and the pack states the exact
  recompute procedure.
